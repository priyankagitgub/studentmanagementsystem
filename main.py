...
class DBHelper():
    ...
    def addStudent(self,roll,name,gender,branch,year,academic_year,address,mobile):
        try:
            ...
            self.c.close()
            self.conn.close()
            QMessageBox.information(QMessageBox(),'Successful','Empolyee is added successfully to the database.')
        except Exception:
            QMessageBox.warning(QMessageBox(), 'Error', 'Could not add Employee to the database.')
    ...
    def searchStudent(self,roll):
        ...
        if not self.data:
            QMessageBox.warning(QMessageBox(), 'Error', 'Could not find any Empolyee with this code '+str(roll))
            return None
        self.list=[]
        for i in range(0,8):
        ...
        try:
            ...
            if not self.c.fetchone():
                ...
                #otherwise make the payment.
                    ...
                    self.c.execute("SELECT * from payments WHERE roll=" + str(roll)+" AND semester=0")
                    if not self.c.fetchone():
                        QMessageBox.warning(QMessageBox(), 'Error','Employee ' + str(roll) + ' has Odd Semester fee payment due.Pay that first.')
                    else:
                        self.c.execute(
                ...
                #here we try to check if admin is trying to make payment for the same semester twice.
                    ...
                    QMessageBox.warning(QMessageBox(), 'Error','Employee with EMP Code ' + str(roll) + ' has already paid this semester fees.')
...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.text = QLabel("Enter the EMP Code of the Empolyee")
        ...
        self.dialog = QDialog()
        self.dialog.setWindowTitle("EMP Code")
        self.dialog.setLayout(self.vbox)

        self.rollForPayment = 0
        self.vboxPayment = QVBoxLayout()
        self.textPayment = QLabel("Enter the EMP Code of the Empolyee")
        ...
        self.dialogPayment.setWindowTitle("EMP Code")
        self.dialogPayment.setLayout(self.vboxPayment)

        self.btnEnterStudent=QPushButton("Enter Employee Details",self)
        self.btnShowStudentDetails=QPushButton("Show Employee Details",self)
        ...
        self.resize(400,280)
        self.setWindowTitle("Employee Database Management System")
    ...
    def showStudent(self):
        if self.editField.text() is "":
            QMessageBox.warning(QMessageBox(), 'Error',
                                'You must give the EMP Code to show the results for.')
            return None
